# bloodhound-collector — Athena MCP Tool Server
...
```bash
make new-tool NAME=bloodhound-collector
```
...
```bash
make dev-tool NAME=bloodhound-collector           # stdio mode
make dev-tool-http NAME=bloodhound-collector      # HTTP mode (port 8090)
```
