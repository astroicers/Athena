# coercion-tools — Athena MCP Tool Server
...
```bash
make new-tool NAME=coercion-tools
```
...
```bash
make dev-tool NAME=coercion-tools           # stdio mode
make dev-tool-http NAME=coercion-tools      # HTTP mode (port 8090)
```
