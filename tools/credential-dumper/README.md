# credential-dumper — Athena MCP Tool Server
...
```bash
make new-tool NAME=credential-dumper
```
...
```bash
make dev-tool NAME=credential-dumper           # stdio mode
make dev-tool-http NAME=credential-dumper      # HTTP mode (port 8090)
```
