# Stage1-Web.ps1 -- Athena AD Lab Stage 1 for WEB01 (Bridged mode)
...
# After this completes, snapshot as 'S2-domain-joined' then run Stage2-Web.ps1.
...
Invoke-Phase 'C-domainjoin' {
    # Point DNS at DC so domain join can resolve corp.athena.lab
    ...
    # Wait for DC to answer (DC may still be rebooting after its own AD DS promo)
    $maxTries = 30
    ...
    if ($needsRename -or $needsJoin) {
        ...
        if ($needsJoin) {
            # NOTE: OU may not exist yet (Stage-2 creates OU=Servers), so join to default Computers container
            if ($needsRename) {
