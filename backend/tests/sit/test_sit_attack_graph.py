...
async def test_rebuild_idempotent(sit_services):
    ...
    assert count_first == count_second, f"rebuild should be idempotent: {count_first} != {count_second}"
...
async def test_explored_node_from_execution(sit_seeded_with_execution, sit_ws_manager):
    ...
    row = await db.fetchrow(
        "SELECT status FROM attack_graph_nodes WHERE operation_id = $1 AND technique_id = 'T1003.001'",
        "test-op-1",
    )
    assert row is not None, "T1003.001 node should exist in attack_graph_nodes"
    assert row["status"] == "explored", f"T1003.001 should be 'explored' after success execution, got '{row['status']}'"
...
async def test_build_orient_summary_format(sit_services):
    ...
    assert "coverage" in summary_lower, f"Orient summary should mention 'coverage', got: {summary[:200]}"
    # 'position' appears when there are explored nodes (seed has 4 explored)
    assert "position" in summary_lower, f"Orient summary should mention 'position', got: {summary[:200]}"
...
async def test_rebuild_broadcasts_ws_event(sit_services):
    ...
    assert "graph.updated" in event_types, f"Expected 'graph.updated' in WS events, got: {event_types}"
