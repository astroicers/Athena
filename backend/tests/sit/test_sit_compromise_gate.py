...
async def test_ssh_credential_marks_compromised(sit_services):
    ...
    sit_services.engine_client.execute = AsyncMock(
        return_value=ExecutionResult(
            success=True,
            execution_id="exec-cg1",
            output="uid=0(root)",
            facts=[],
        )
    )
    ...
    row = await db.fetchrow("SELECT is_compromised FROM targets WHERE id = 'test-target-1'")
...
async def test_recon_success_no_compromise(sit_services):
    ...
    await db.execute("UPDATE targets SET is_compromised = FALSE WHERE id = 'test-target-1'")
...
async def test_success_activates_pending_agent(sit_services):
    ...
    agent_row = await db.fetchrow("SELECT status FROM agents WHERE paw = 'agent-paw-001'")
...
async def test_techniques_executed_increments(sit_services):
    ...
    before = await db.fetchval("SELECT techniques_executed FROM operations WHERE id = 'test-op-1'")
    ...
    after = await db.fetchval("SELECT techniques_executed FROM operations WHERE id = 'test-op-1'")
