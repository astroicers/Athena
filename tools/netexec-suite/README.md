# netexec-suite — Athena MCP Tool Server
...
```bash
make new-tool NAME=netexec-suite
```
...
```bash
make dev-tool NAME=netexec-suite           # stdio mode
make dev-tool-http NAME=netexec-suite      # HTTP mode (port 8090)
```
