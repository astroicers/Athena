    ...
    <div className={`shrink-0 w-8 h-8 rounded-athena flex items-center justify-center ${bgColor}`}>
      {isScanning ? (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="animate-spin" style={{ animationDuration: "3s" }}>
          <circle cx="10" cy="10" r="7" stroke="currentColor" strokeWidth="1.5" strokeDasharray="22 22" className="text-athena-accent" />
          <circle cx="10" cy="10" r="2" fill="currentColor" className="text-athena-accent" />
        </svg>
      ) : isCompromised ? (
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-mono font-bold text-athena-text-light">
              {hostname}
            </span>
          ...
          <div className="space-y-1 text-xs font-mono text-athena-text-tertiary">
            <div className="flex justify-between">
              <span>{t("ip")}</span>
              <span className="text-athena-text-light">{ipAddress}</span>
            </div>
            <div className="flex justify-between">
              <span>{t("role")}</span>
              <span className="text-athena-text-light">{role}</span>
            </div>
            {privilegeLevel && (
              <div className="flex justify-between">
            ...
            <div className="mt-2 pt-2 border-t border-athena-border/30 space-y-1">
              {os && (
                <div className="flex justify-between text-xs font-mono">
                  ...
                  <span className="text-athena-text-light">{os}</span>
                </div>
              )}
              <div className="flex justify-between text-xs font-mono">
                <span className="text-athena-text-tertiary">{t("ports")}</span>
              ...
              {services && services.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {services.slice(0, 3).map((svc) => (
                    <span
                      key={svc.port}
                      className="text-sm font-mono bg-athena-bg border border-athena-border/50 rounded-athena px-1.5 py-0.5 text-athena-text-tertiary"
                    >
              ...
              {lastScanAt && (
                <div className="text-sm font-mono text-athena-text-tertiary text-right">
          ...
          {isScanning && (
            <div className="mt-2 space-y-1">
              <div className="flex items-center justify-between text-sm font-mono">
                <span className="text-athena-accent animate-pulse">
                  {scanPhase
