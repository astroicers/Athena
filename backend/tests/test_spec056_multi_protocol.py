...
class TestProtocolMapMatching:
    ...
    def test_mysql_service_matches(self):
        ...
        assert any(
            s["port"] == mysql_entry["port"] and s["service"] in mysql_entry["service_keywords"] for s in services
        )
    ...
    def test_postgresql_service_matches(self):
        ...
        assert any(s["port"] == pg_entry["port"] and s["service"] in pg_entry["service_keywords"] for s in services)
    ...
    def test_ftp_service_matches(self):
        ...
        assert any(s["port"] == ftp_entry["port"] and s["service"] in ftp_entry["service_keywords"] for s in services)
