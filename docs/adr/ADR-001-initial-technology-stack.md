...
### Rev 1 — Phase B（2026-03-02）：執行引擎與 AI 情報層重大更新

| 領域 | 原始決策 | 更新後狀態 |
|------|---------|-----------|
| AI/ML | PentestGPT（MIT）+ Claude（主）+ GPT-4（備） | **OrientEngine（自研，直接呼叫 Claude API）** — PentestGPT 為概念借鑒，實際無程式碼依賴；GPT-4 fallback 移除 |
| 執行引擎 | Caldera（必要）+ Shannon（選用 AGPL-3.0） | **DirectSSHEngine（預設）** + CalderaClient（向後相容選用）— Shannon 從未呼叫，移除；Caldera 降為可選 |

**更新理由：**
- OrientEngine 在 Phase 5 實作時即為完全自研（`orient_engine.py`），從未依賴 PentestGPT 程式碼
- Caldera 的「需 alive agent 才能執行」設計與 Athena「SSH 成功 → 自動推進」哲學矛盾
- Shannon AGPL-3.0 隔離維護成本高，且 `SHANNON_URL` 預設為空、從未真實呼叫
- DirectSSHEngine 讓 Athena 無需部署任何外部 C2，降低系統複雜度

**參考：** ADR-017（DirectSSHEngine 架構）、ADR-018（Technique Playbook 知識庫）
