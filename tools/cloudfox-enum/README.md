# cloudfox-enum — Athena MCP Tool Server
...
```bash
make new-tool NAME=cloudfox-enum
```
...
```bash
make dev-tool NAME=cloudfox-enum           # stdio mode
make dev-tool-http NAME=cloudfox-enum      # HTTP mode (port 8090)
```
