...
Athena 是一套以 **OODA 決策迴圈**驅動的自主攻擊平台。在 scope / opsec / RoE 三重合規閘下，它於**授權範圍內**自主推進偵查 → 突破 → 拿 shell → 提權 → 橫移 → 網域層級後滲透——**下一步打哪台、用哪招，是迴圈自己決定的，不是人工逐步下令**。它對內以 MCP 驅動資安工具容器；決策腦可換成地端本地 LLM，整套可 **air-gap** 部署。
...
- **自主 OODA 攻擊迴圈** — 從偵查到網域層級後滲透（授權範圍內），全程迴圈自決、人不逐步下令；可觀測、可打斷、可重試。
- **MCP 工具編排** — 對內以 MCP 驅動資安工具容器（nmap / msf / impacket 等）。對外作為 MCP server 受外部指揮系統納管為 **roadmap 規劃中**，非目前已交付能力。
...
> 上述為**授權範圍內**的平台能力；端到端演示素材後補。商業評估與 POC 可於授權靶場安排實測；細節洽談取得。
...
- **MCP 工具編排 + 決策引擎熱插拔**：能納管你的工具容器；換戰法不重寫（對外被上游指揮系統反向納管為 roadmap 規劃中）。
