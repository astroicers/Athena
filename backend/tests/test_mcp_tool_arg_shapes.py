...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dns_resolve", "sub1.example.com,sub2.example.com", "subdomains", "sub1.example.com,sub2.example.com"),
...
def test_tool_arg_shape(
    ...
    assert expected_key in args, f"Expected key '{expected_key}' in args for tool '{tool_name}', got {args}"
...
def test_dns_resolve_does_not_include_target_key() -> None:
    ...
    assert "target" not in args, "dns_resolve shape must NOT include 'target' key — it expects 'subdomains' only"
...
def test_tool_arg_shapes_is_dict_of_callables() -> None:
    ...
    for name, shaper in _TOOL_ARG_SHAPES.items():
        ...
        assert isinstance(result, dict), (
            f"_TOOL_ARG_SHAPES[{name!r}]('test') returned {type(result).__name__}, expected dict"
        )
